...
public class Scoreboard {
    ...
    public void add(GameEntry entry) {
        int size = board.size();
        Double entryScore = (double)entry.getScore();
        /** if score board isn't full : */
        while (size > 0 && entryScore >= board.get(size - 1).getScore()){
            size--;
        }
        board.insert(entry, size);
        /** if score board is over full : */
        while (capacity < board.size()){
            board.removeLast();
        }
        /** Unsure when to throw this provided exception
    ...
    public GameEntry remove(int i) throws IndexOutOfBoundsException {
        if (i >= board.size() || i < 0) throw new IndexOutOfBoundsException("Cannot remove GameEntry at index " + i + ". No such index exists.");
        else{
